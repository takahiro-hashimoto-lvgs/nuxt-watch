...
watchEffect( () => {
  console.log('AsyncWatchEffect', 'count', count.value, 'unwatch_count', unwatch_count.value);
});
const onClick = async () => {
  setTimeout(() => {
    count.value++;
    console.log('AsyncWatchEffect didIncrement', 'count', count.value);
  }, 3000);
};
const onClickSame = () => {
  setTimeout(() => {
...
const onClick3 = async () => {
  setTimeout(() => {
    count.value+=2;
    console.log('AsyncWatchEffect didIncrement2', 'count', count.value);
  }, 1000);
};
const onUnwatchClick = async () => {
  setTimeout(() => {
    unwatch_count.value++;
    console.log('AsyncWatchEffect unwatched didIncrement', 'unwatch_count', unwatch_count.value);
  }, 1000);
};
const onUnwatchNpDepClick = async () => {
  setTimeout(() => {
    unwatched_count_nodep.value++;
    console.log('AsyncWatchEffect unwatched_count_nodep didIncrement', 'unwatched_count_nodep', unwatched_count_nodep.value);
  }, 1000);
};
</script>
